...
3. **展開タイミング（v0.14.0+）**: `makezip`/`export` は `role`・`function_name`・`environment`・`tags`・`permissions`・`vpc`・`function_url`（CORS含む）などスクリプトに出力される全フィールドの ${VAR} 展開を生成スクリプトの実行時まで遅延させる（同一の ZIP + スクリプトを複数環境に使い回すため）。`info`/`delete` は AWS を直接呼び出すため、コマンド実行時に即時展開する
...
- 権限設定（add-permission）
- ロググループ作成＆保持期間設定
- jq があれば結果整形（無ければそのまま出力）
- スクリプトに出力される全フィールドの ${VAR} はスクリプト実行時に展開（冒頭で未設定チェック、JSON埋め込み箇所はエスケープ処理付き）
- ZIP はスクリプトでは削除しない（deploy コマンドが後始末する）
